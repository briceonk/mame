...
TIMER_CALLBACK_MEMBER(ews4800_state::dma_check)
{
...
void ews4800_state::intc_clear_w(offs_t offset, uint32_t data)
{
...
uint32_t ews4800_state::intc_status_r(offs_t offset)
{
	LOG("intc_status r 0x%x (%s)\n", intc_status, machine().describe_context());
	return intc_status;
}

void ews4800_state::intc_mask_w(offs_t offset, uint32_t data)
{
...
uint32_t ews4800_state::intc_mask_r(offs_t offset)
{
...
TIMER_CALLBACK_MEMBER(ews4800_state::timer0_clock)
{
...
{
	m_timer0_timer = machine().scheduler().timer_alloc(timer_expired_delegate(FUNC(ews4800_state::timer0_clock), this));
	m_dma_check = machine().scheduler().timer_alloc(timer_expired_delegate(FUNC(ews4800_state::dma_check), this));
}
...
void ews4800_state::patch_rom(address_map &map)
{
	// Bypass checksum
...
void ews4800_state::cpu_map(address_map &map)
{
	...
	map(0x1e000000, 0x1e000003).w(FUNC(ews4800_state::intc_clear_w));
	map(0x1e000004, 0x1e000007).r(FUNC(ews4800_state::intc_status_r));
	map(0x1e000008, 0x1e00000b).rw(FUNC(ews4800_state::intc_mask_r), FUNC(ews4800_state::intc_mask_w));
	...
	map(0x1e440000, 0x1e44000f)
		.rw(m_scc[0], FUNC(z80scc_device::ab_dc_r), FUNC(z80scc_device::ab_dc_w))
		.umask32(0xff000000);
	map(0x1e480000, 0x1e48000f).rw(m_scc[1], FUNC(z80scc_device::ab_dc_r), FUNC(z80scc_device::ab_dc_w)).umask32(0xff000000);
...
void ews4800_state::lance_irq(int state)
{
...
void ews4800_state::scsi_irq(int state)
{
...
void ews4800_state::scsi_drq_w(int state)
{
...
void ews4800_state::ews4800_310(machine_config &config)
{
	...
	NSCSI_CONNECTOR(config, "scsi:7").option_set("ncr53c96", NCR53C94).clock(24_MHz_XTAL).machine_config([this](device_t *device)
			...
			adapter.irq_handler_cb().set(*this, FUNC(ews4800_state::scsi_irq));
			adapter.drq_handler_cb().set(*this, FUNC(ews4800_state::scsi_drq_w)); 
			});

	// ethernet
	AM7990(config, m_net);
	m_net->intr_out().set(FUNC(ews4800_state::lance_irq));
