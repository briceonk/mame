...
// copyright-holders:Olivier Galibert,Brice Onken
...
DEFINE_DEVICE_TYPE(SPIFI, spifi_device, "spifi", "HP 1TV3-0302 SCSI Protocol Controller")

void spifi_device::map(address_map &map)
{
	map(0x0, 0x0).rw(FUNC(spifi_device::tcounter_lo_r), FUNC(spifi_device::tcount_lo_w));
	map(0x1, 0x1).rw(FUNC(spifi_device::tcounter_hi_r), FUNC(spifi_device::tcount_hi_w));
	map(0x2, 0x2).rw(FUNC(spifi_device::fifo_r), FUNC(spifi_device::fifo_w));
	map(0x3, 0x3).rw(FUNC(spifi_device::command_r), FUNC(spifi_device::command_w));
	map(0x4, 0x4).rw(FUNC(spifi_device::status_r), FUNC(spifi_device::bus_id_w));
	map(0x5, 0x5).rw(FUNC(spifi_device::istatus_r), FUNC(spifi_device::timeout_w));
	map(0x6, 0x6).rw(FUNC(spifi_device::seq_step_r), FUNC(spifi_device::sync_period_w));
	map(0x7, 0x7).rw(FUNC(spifi_device::fifo_flags_r), FUNC(spifi_device::sync_offset_w));
	map(0x8, 0x8).rw(FUNC(spifi_device::conf_r), FUNC(spifi_device::conf_w));
	map(0xa, 0xa).w(FUNC(spifi_device::test_w));
	map(0x9, 0x9).w(FUNC(spifi_device::clock_w));
}

uint8_t spifi_device::read(offs_t offset)
{
	switch (offset)
	{
...
void spifi_device::write(offs_t offset, uint8_t data)
{
	switch (offset)
	{
...
spifi_device::spifi_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock)
...
spifi_device::spifi_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock)
	: spifi_device(mconfig, SPIFI, tag, owner, clock)
{
}

void spifi_device::device_start()
{
...
void spifi_device::device_reset()
{
	fifo_pos = 0;
...
void spifi_device::reset_disconnect()
{
	scsi_bus->ctrl_w(scsi_refid, 0, ~S_RST);
...
void spifi_device::scsi_ctrl_changed()
{
...
void spifi_device::device_timer(emu_timer &timer, device_timer_id id, int param, void *ptr)
{
	step(true);
}

void spifi_device::step(bool timeout)
{
	uint32_t ctrl = scsi_bus->ctrl_r();
	...
	case ARB_COMPLETE << SUB_SHIFT: {
		...
		if(win != scsi_id) {
			...
			fatalerror("spifi_device::step need to wait for bus free\n");
		}
...
void spifi_device::send_byte()
{
	if(!fifo_pos)
		fatalerror("spifi_device::send_byte - !fifo_pos\n");
...
void spifi_device::function_bus_complete()
{
...
void spifi_device::function_complete()
{
...
void spifi_device::bus_complete()
{
	LOG("bus_complete\n");
	state = IDLE;
	istatus |= I_BUS;
...
void spifi_device::delay(int cycles)
{
	if(!clock_conv)
		return;
	cycles *= clock_conv;
...
void spifi_device::delay_cycles(int cycles)
{
	tm->adjust(clocks_to_attotime(cycles));
}

uint8_t spifi_device::tcounter_lo_r()
{
	LOG("tcounter_lo_r %02x (%s)\n", tcounter & 0xff, machine().describe_context());
	return tcounter;
}

void spifi_device::tcount_lo_w(uint8_t data)
{
	tcount = (tcount & 0xff00) | data;
	LOG("tcount_lo_w %02x (%s)\n", data, machine().describe_context());
}

uint8_t spifi_device::tcounter_hi_r()
{
	LOG("tcounter_hi_r %02x (%s)\n", tcounter >> 8, machine().describe_context());
	return tcounter >> 8;
}

void spifi_device::tcount_hi_w(uint8_t data)
{
	tcount = (tcount & 0x00ff) | (data << 8);
	LOG("tcount_hi_w %02x (%s)\n", data, machine().describe_context());
}

uint8_t spifi_device::fifo_pop()
{
	uint8_t r = fifo[0];
	fifo_pos--;
...
void spifi_device::fifo_push(uint8_t val)
{
	fifo[fifo_pos++] = val;
	check_drq();
}

uint8_t spifi_device::fifo_r()
{
	uint8_t r;
	if(fifo_pos) {
		r = fifo[0];
...
void spifi_device::fifo_w(uint8_t data)
{
...
uint8_t spifi_device::command_r()
{
	LOG("command_r (%s)\n", machine().describe_context());
	return command[0];
}

void spifi_device::command_w(uint8_t data)
{
...
void spifi_device::command_pop_and_chain()
{
	if(command_pos) {
		command_pos--;
		if(command_pos) {
...
void spifi_device::start_command()
{
	uint8_t c = command[0] & 0x7f;
	...
	default:
		fatalerror("spifi_device::start_command unimplemented command %02x\n", c);
	}
}

bool spifi_device::check_valid_command(uint8_t cmd)
{
	int subcmd = cmd & 15;
	switch((cmd >> 4) & 7) {
...
void spifi_device::bus_id_w(uint8_t data)
{
	bus_id = data & 7;
	LOG("bus_id=%d\n", bus_id);
}

uint8_t spifi_device::istatus_r()
{
	uint8_t res = istatus;
...
void spifi_device::timeout_w(uint8_t data)
{
	LOG("timeout_w 0x%02x\n", data);
	select_timeout = data;
}

uint8_t spifi_device::seq_step_r()
{
	LOG("seq_step_r %d (%s)\n", seq, machine().describe_context());
	return seq;
}

void spifi_device::sync_period_w(uint8_t data)
{
	sync_period = data & 0x1f;
}

uint8_t spifi_device::fifo_flags_r()
{
	return fifo_pos;
}

void spifi_device::sync_offset_w(uint8_t data)
{
	sync_offset = data & 0x0f;
}

uint8_t spifi_device::conf_r()
{
	return config;
}

void spifi_device::conf_w(uint8_t data)
{
	config = data;
	scsi_id = data & 7;
...
void spifi_device::test_w(uint8_t data)
{
	if (test_mode)
		logerror("test_w %d (%s) - test mode not implemented\n", data, machine().describe_context());
}

void spifi_device::clock_w(uint8_t data)
{
	clock_conv = data & 0x07;
}

void spifi_device::dma_set(int dir)
{
	dma_dir = dir;
...
void spifi_device::dma_w(uint8_t val)
{
	fifo_push(val);
	decrement_tcounter();
	check_drq();
...
void spifi_device::decrement_tcounter(int count)
{
	if (!dma_command)
		return;
...
uint8_t spifi_device::auxctrl_r(offs_t offset)
{
	LOG("read spifi_reg.auxctrl = 0x%x\n", spifi_reg.auxctrl);
	return spifi_reg.auxctrl;
}

void spifi_device::auxctrl_w(offs_t offset, uint8_t data)
{
	LOG("write spifi_reg.auxctrl = 0x%x ", data);
...
/*
SPIFI reset
[:scsi0:7:spifi3] write spifi_reg.auxctrl = 0x80
[:scsi0:7:spifi3] SRST asserted
[:scsi0:7:spifi3] write spifi_reg.auxctrl = 0x40
[:scsi0:7:spifi3] CRST asserted
[:scsi0:7:spifi3] write spifi_reg.auxctrl = 0x20
[:scsi0:7:spifi3] SETRST asserted
[:scsi0:7:spifi3] write spifi_reg.auxctrl = 0x0
[:scsi0:7:spifi3] write spifi_reg.auxctrl = 0x80
[:scsi0:7:spifi3] SRST asserted
[:scsi0:7:spifi3] write spifi_reg.auxctrl = 0x40
[:scsi0:7:spifi3] CRST asserted
[:scsi0:7:spifi3] write spifi_reg.auxctrl = 0x4
[:scsi0:7:spifi3] DMAEDGE asserted
[:scsi0:7:spifi3] write spifi_reg.imask = 0x22
[:scsi0:7:spifi3] write spifi_reg.config = 0xf
[:scsi0:7:spifi3] write spifi_reg.fastwide = 0x0
[:scsi0:7:spifi3] write spifi_reg.prctrl = 0x0
[:scsi0:7:spifi3] write spifi_reg.loopctrl = 0x0
[:] LED_DISK: ON

Transaction setup?
[:scsi0:7:spifi3] read spifi_reg.spstat = 0x0
[:scsi0:7:spifi3] write spifi_reg.identify = 0x80
[:scsi0:7:spifi3] write spifi_reg.count_hi = 0x0
[:scsi0:7:spifi3] write spifi_reg.count_mid = 0x0
[:scsi0:7:spifi3] write spifi_reg.count_low = 0x0
[:scsi0:7:spifi3] write spifi_reg.count_hi = 0x0
[:scsi0:7:spifi3] write spifi_reg.count_mid = 0x0
[:scsi0:7:spifi3] write spifi_reg.count_low = 0x24
[:scsi0:7:spifi3] write spifi_reg.fifoctrl = 0x0
[:scsi0:7:spifi3] write spifi_reg.autodata = 0xc0

DMAC setup
[:dmac] dmac3-0 csr_r: 0x0
[:dmac] dmac3-0 length_w: 0x24
[:dmac] dmac3-0 address_w: 0xd60
[:dmac] dmac3-0 intr_w: 0x202
[:dmac] dmac3-0 csr_w: 0x3

Enter INQUIRY command into command buffer
[:scsi0:7:spifi3] SPIFI3: cmd_buf_w(0x70, 0x12)
[:scsi0:7:spifi3] SPIFI3: cmd_buf_w(0x71, 0x0)
[:scsi0:7:spifi3] SPIFI3: cmd_buf_w(0x72, 0x0)
[:scsi0:7:spifi3] SPIFI3: cmd_buf_w(0x73, 0x0)
[:scsi0:7:spifi3] SPIFI3: cmd_buf_w(0x74, 0x24)
[:scsi0:7:spifi3] SPIFI3: cmd_buf_w(0x75, 0x0)

Set a few more SPIFI params
[:scsi0:7:spifi3] write spifi_reg.cmlen = 0xc6
[:scsi0:7:spifi3] write spifi_reg.autostat = 0x1
[:scsi0:7:spifi3] write spifi_reg.select = 0x8c

loop until interrupt
[:dmac] dmac3-0 intr_r: 0x203
[:dmac] dmac3-0 intr_r: 0x203
[:dmac] dmac3-0 intr_w: 0x203
[:scsi0:7:spifi3] read spifi_reg.intr = 0x0 (':cpu' (9FC15DF0))
[:scsi0:7:spifi3] read spifi_reg.spstat = 0x0
[:scsi0:7:spifi3] read spifi_reg.init_status = 0x0
[:scsi0:7:spifi3] read spifi_reg.intr = 0x0 (':cpu' (9FC15DF0))
[:dmac] dmac3-0 intr_r: 0x203
[:dmac] dmac3-0 intr_r: 0x203
[:dmac] dmac3-0 intr_w: 0x203
