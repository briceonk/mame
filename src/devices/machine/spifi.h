...
#ifndef MAME_MACHINE_SPIFI_H
#define MAME_MACHINE_SPIFI_H

#pragma once

#include "machine/nscsi_bus.h"

class spifi_device : public nscsi_device, public nscsi_slot_card_interface
{
public:
	spifi_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock);
...
protected:
	spifi_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock);
	...
	uint8_t auxctrl_r(offs_t offset);
	void auxctrl_w(offs_t offset, uint8_t data);

	struct spifi_cmd_entry
	{
		// NetBSD has these mapped as uint32_t to align the accesses and such
		// in reality, these are all 8-bit values that are mapped, in typical NWS-5000 series fashion, to be 32-bit word aligned.
...
DECLARE_DEVICE_TYPE(SPIFI, spifi_device)

#endif // MAME_MACHINE_SPIFI_H
